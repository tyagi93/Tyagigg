...
async def lookup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Strict Limit Checking Logic (Blocks immediately if 0 or below)
    if not premium:
    ...
    try:
                    ...
                    for idx, item in enumerate(records, 1):
                        ...
                        for k, v in item.items():
                            if v is not None and str(v).strip() != "" and str(v).upper() != "NA":
                                # Exclude any backend injected footer fields
        ...
        # Stripped old footers, only your absolute configuration is generated here
        report_text += f"🤖 Bot: {BOT_NAME}\n"
        report_text += f"👑 Admin: {SUPPORT}"

        # Clean replacement logic for fallback checks
