...
try:
    ...
    BOT_NAME = "NEXO NUMBOT"

# Administrative Settings (Configured for @TYAGI8)
ADMIN_ID = 5744767539
SUPPORT = "@TYAGI8"
DB = "bot.db"
...
async def lookup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            text = f"⚠️ Error: API Data issue (Status Code: {r.status_code})\n\n👑 Owner: {SUPPORT}"

        if "@kihoerack" in text or "@YeuIin" in text:
            text = text.replace("@kihoerack", f"@{context.bot.username}")
