...
def run_web_server():
    # Render is default port (8080) par incoming traffic check karega
...
async def lookup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 3-Limit Strict Check (Strictly Blocks if free searches reach 0 or less)
    if not premium:
        if user and user[3] <= 0:
            await update.message.reply_text(
                "❌ Limit Exhausted: Aapki 3 Free search limits khatam ho chuki hain!\n\n"
                "Kripya aage search karne ke liye Premium active karein. Plan details dekhne ke liye /start bhej kar 'Premium' button par click karein."
            )
            return
                    ...
                    for idx, item in enumerate(records, 1):
                        ...
                        for k, v in item.items():
                            if v is not None and str(v).strip() != "" and str(v).upper() != "NA":
                                if k.lower() not in ["cached", "owner", "admin"]:
