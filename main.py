...
# 🔑 GROQ FREE API KEY (Sahi jagah apply kar di gayi hai)
...
# Keep-Alive Server for Render (24/7 online setup)
web_app = Flask('')
@web_app.route('/')
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"👋 Welcome to {BOT_NAME}\n\nSearch karne ke liye pehle niche se koi ek button select karein.",
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------- FREE AI SEARCH (GROQ LLAMA) ----------------
        ...
        if not ai_client:
            await update.message.reply_text("⚠️ AI System Setup Error: Groq client load nahi ho paya.")
            return
            
        await context.bot.send_chat_action(chat_id=user_id, action="typing")
        try:
            # Llama model call for smart response
