...
# 🔑 GROQ FREE API KEY (Aapki key perfectly lagayi hui hai)
...
# ==========================================
# 📊 NEW THREE BUTTONS LAYOUT (FIXED)
# ==========================================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Saare mandatory buttons: Number Info, Vehicle Info, aur AI Search add kar diye hain
    keyboard = [
    ...
    await update.message.reply_text(
        f"Welcome to {BOT_NAME}\n\nKripya pehle niche diye gaye buttons mein se koi ek select karein:",
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "lookup_mode":
        ...
        await query.message.reply_text("Menu panel active hai.")
    elif query.data == "profile":
        await query.message.reply_text(f"Profile Info:\n\nUser ID: {user_id}\nBot: {BOT_USERNAME_SIGNATURE}")
    elif query.data == "premium":
        await query.message.reply_text(f"Premium Subscription ke liye support par message karein: {SUPPORT}")

# ==========================================
# STRICT SYSTEM MESSAGE ROUTER
# ==========================================
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🛑 Strict Check: Pehle button select karna zaroori hai
    if not state:
        await update.message.reply_text("Notice: Kripya pehle upar diye gaye buttons mein se Number Info, Vehicle Info ya AI Search select karein!")
        return
    ...
    if state == "NUMBER_SEARCH":
        ...
        try:
            r = requests.get(f"{API_URL}&search={user_input}", timeout=20)
            if r.status_code == 200:
                await update.message.reply_text(f"Search Result:\n\n{r.text[:1500]}")
            else:
                await update.message.reply_text(f"API Error (Status: {r.status_code})")
    ...
    # ---------------- VEHICLE SEARCH ----------------
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                await update.message.reply_text(f"Vehicle API Error (Status: {r.status_code})")
        except Exception as e:
            await update.message.reply_text(f"Connection Error: {e}")
        USER_STATES[user_id] = None 
        return

    # ---------------- FREE AI SEARCH (GROQ LLAMA) ----------------
    elif state == "AI_SEARCH":
        if not ai_client:
            await update.message.reply_text("AI System Setup Error: Groq client load nahi ho paya.")
            return
        ...
        try:
            ...
            await update.message.reply_text(f"AI Server Error: {e}")
        USER_STATES[user_id] = None 
        return
